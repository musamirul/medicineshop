...
<?php 
    //Count All
    $query_countall = mysqli_query($con,"SELECT count(*) FROM customer");
    $result_countall = mysqli_fetch_array($query_countall);
    $allcount = $result_countall[0];

    //Count active
    $query_countactive = mysqli_query($con,"SELECT count(*) FROM customer WHERE Cust_Status = 'Active'");
    $result_countactive = mysqli_fetch_array($query_countactive);
    $activecount = $result_countactive[0];

    //count deactive
    $query_countdeactive = mysqli_query($con,"SELECT count(*) FROM customer WHERE Cust_Status = 'deactive'");
    $result_countdeactive = mysqli_fetch_array($query_countdeactive);
    $deactivecount = $result_countdeactive[0];

    //count suspend
    $query_countsuspend = mysqli_query($con,"SELECT count(*) FROM customer WHERE Cust_Status = 'suspend'");
        ...
        <div class="row p-3">
            <div class="d-flex flex-row">
                ...
                <div class="text-start ms-3">
                    <span style="font-size: 23px;font-weight: bold;">Customer Management</span> <br/>
                    <span style="font-size: 14px; color: grey;">View and manage Customer</span>
                </div>
            </div>
        </div>
            ...
            <div class="col">
                <div class="row bg-white p-3">
                    <ul class="nav nav-pills d-flex justify-content-start">
                        ...
                        <li style="width: 170px;" class="nav-item">
                            <a class="nav-link active" aria-current="page" href="Admin_ManageCust_Suspend.php"><center>Suspend<span class="shadow badge bg-secondary ms-3"><?php echo $suspendcount ?></span></center></a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <table id="example" class="display center" style="width: 100%; text-align: center;">
                ...
                <tbody>
                    <?php
                        ...
                        $query_showData = mysqli_query($con,"SELECT * FROM customer WHERE Cust_Status = 'suspend'");
                        while($result_showData = mysqli_fetch_array($query_showData)){
                        $Cust_ID = $result_showData['Cust_ID'] 
                    ?>
                    <tr>
                        <td><?php echo $result_showData['Cust_ID']; ?></td>
...
<?php
    if(isset($_POST['saveDetail'])){
        ...
        echo '<script>window.location.href="Admin_ManageCust_Suspend.php"</script>';
    }
?>
